...
class PluginUpdateChecker_3_0 {
	...
	protected function installHooks(){
		...
		add_filter('plugin_row_meta', array($this, 'addCheckForUpdatesLink'), 100, 2);
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		$products = $aebaseapi->get_products_plugin_files();

		if ( $isRelevant && current_user_can('update_plugins') && in_array($pluginFile, $products) ) {
			...
			if( !ae_updater_validate_code( $plugin_slug, $purchase_code ) ){
				...
				$row_meta = array(
					...
					'ae_updates' => '<a class="ae-plugin-update-row-meta-register-for-updates" href="' . $url . '">' . __( 'Register for Updates', 'create-customer-order' ) . '</a>'  .  '<span class="ae-plugin-update-row-meta-register-for-updates-inline"><span class="ae-plugin-update-row-meta-divider"></span>' . sprintf( __( '<em><a href="%s">Register here</a> to get this update</em>', 'create-customer-order' ), $url ) . '</span>',

				);
				$pluginMeta = array_merge( $pluginMeta, $row_meta );
