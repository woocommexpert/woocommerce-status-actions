...
;(function ($) {
	...
	function Plugin(element, options) {
		...
		this.iconPicker = $('<div/>', {
			...
			html:       '<div class="selector">' +
							...
							'</span>' +
							'<span class="selector-button" title="Select Icon">'+
							'</span>' +
						 '</div>' +
						 '<div class="selector-popup" style="display: none;">' + ((this.settings.hasSearch) ?
							 ...
							 '<div class="selector-category">' +
								 '<select name="" class="wc-enhanced-select icon-category-select" style="display: none">' +
								 '</select>' +
							 '</div>' +
	...
	Plugin.prototype = {
		...
		loadCategories: function() {
			...
			this.selectCategory.val('all').trigger('change');
		},
		...
		renderIconContainer: function () {
			...
			for (var i = 0, item; item = iconsPaged[i++];) {
				...
				// Set the icon box
				$('<span/>', {
					html:      '<i data-fip-value="' + item + '" ' + (this.settings.useAttribute ? (this.settings.attributeName + '="' + ( this.settings.convertToHex ? '&#x' + parseInt(item, 10).toString(16) + ';' : item ) + '"') : 'class="' + this.settings.prefix_class + item + '"') + '></i>',
					'class':   'fip-box',
					title: flipBoxTitle
		...
		setSelectedIcon: function (theIcon) {
			...
			// Use class
				...
				this.iconPicker.find('.selected-icon').html('<i class="' + this.settings.prefix_class + (theIcon || 'fip-icon-block') + '"></i>');
			}
